...
int main(void) {
    ...
    while (1) {                // Infinitely loop in main function
        ...
        /*if (MEAS_data_ready) {			// Show data if new data
        available MEAS_data_ready = false; MEAS_show_data();
        }*/

        /*if (PB_pressed()) {				// Check if user
        pushbutton was pressed DAC_active = !DAC_active;	// Toggle DAC
        on/off if (DAC_active) { DAC_init(); BSP_LED_On(LED4); } else {
                        DAC_reset();
                        BSP_LED_Off(LED4);
                }
        }*/
